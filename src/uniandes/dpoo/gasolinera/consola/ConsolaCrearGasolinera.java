...
public class ConsolaCrearGasolinera extends ConsolaBasica {
	 ...
	 */
	private final String[] opcionesCrearGasolinera = new String[] { "Nuevo tipo de gasolina", "Nuevo empleado",
			"Cambiar cantidad surtidores", "Crear gasolinera", "Regresar sin crear una gasolinera" };

	/**
	 * Acá se almacenan temporalmente los tipos de gasolina mientras se está
	 * configurando una nueva gasolinera
	 */
	private List<TipoGasolina> tiposGasolina = new ArrayList<TipoGasolina>();

	/**
	 * Acá se almacenan temporalmente los nombres de los empleados mientras se está
	 * configurando una nueva gasolinera
	 */
	private List<String> nombresEmpleados = new ArrayList<String>();

	/**
	 * Esta es la cantidad de surtidores que deberá tener la nueva gasolinera cuando
	 * se construya
	 */
	 ...
	 * 
	 * @return Retorna una nueva gasolinera creada con la configuración dada, o
	 *         retorna null si el usuario no quiso crear una nueva gasolinera
	 */
	public Gasolinera mostrarOpciones() {
		...
		while (nuevaGasolinera == null && !regresar) {
			mostrarEstadoActual(this.cantidadSurtidores, this.tiposGasolina,
					this.nombresEmpleados.toArray(new String[] {}));
			...
			if (opcionSeleccionada == 1) {
				...
				if (todoOk)
					nuevaGasolinera = new Gasolinera(cantidadSurtidores, tiposGasolina,
							this.nombresEmpleados.toArray(new String[] {}));
	...
	/**
	 * En este método se le preguntan al usuario los detalles de un nuevo tipo de
	 * gasolina, y se agrega el nuevo tipo a la lista de tipos temporales.
	 * 
	...
	private void agregarTipoGasolina() {
		...
		if (!nombreValido) {
			...
			double cantidad = pedirNumeroAlUsuario(
					"Digite la cantidad de galones disponibles del nuevo tipo de gasolina");
	...
	/**
	 * En este método se le pregunta al usuario el nombre de un nuevo empleado y se
	 * almacena en la lista de nombres temporales
	 */
	...
	/**
	 * En este método se le pregunta al usuario la cantidad de surtidores que habrá
	 * en la gasolinera y se revisa que el número sea válido
	 */
